...
class LønnsvekstTabell extends React.Component {
    constructor(props){
        super(props)
        this.state = {   
    ...
    componentDidMount() {
        fetch('https://pensjon-regler-'+this.props.valgtMiljø+'.dev.adeo.no/api/lønnsvekstSats?Aktiv='+this.props.aktiv+'&Satstabell='+this.props.currentTabell
        ,{
          headers : { 
          ...
          .then(
            (result) => {
              ...
              console.log("lønnsvekstSats lastet"));
            },
            (error) => {
              this.setState({
                isLoaded: true,
        ...
        return(
            <div>
              <div onClick = {this.handleClick}>
                <Satsheader headline = "Lønnsvekst" show = {this.state.show}></Satsheader>
              </div>
