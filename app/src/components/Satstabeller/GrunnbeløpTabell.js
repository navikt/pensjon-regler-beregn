...
class GrunnbeløpTabell extends React.Component {
        constructor(props){
            super(props)
            this.state = {   
        ...
        componentDidMount() {
            fetch('https://pensjon-regler-'+this.props.valgtMiljø+'.dev.adeo.no/api/grunnbeløpSats?Aktiv='+this.props.aktiv+'&Satstabell='+this.props.currentTabell
            ,{
              headers : { 
              ...
              .then(
                (result) => {
                  ...
                  console.log("grunnbeløpSats lastet"));
                },
                (error) => {
                  this.setState({
                    isLoaded: true,
